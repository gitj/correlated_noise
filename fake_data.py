...
class FakeKIDData(object):
    ...
    def regenerate(self):
        ...
        self.total_signal = np.fft.irfft((red_fft[:,None]*np.sqrt(self.shot_amplitudes/self.shot_amplitudes.max())+shot_fft)/((1+1j*fft_freq/(self.rolloff_freq))[:,None])+amplifier_fft,axis=0)[:self.num_samples,:]
        self.signal_without_red_noise = np.fft.irfft((shot_fft)/((1+1j*fft_freq/(self.rolloff_freq))[:,None])+amplifier_fft,axis=0)[:self.num_samples,:]